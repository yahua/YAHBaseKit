...
#define YAHLog(format, ...) NSLog(format, ## __VA_ARGS__)
#else
#   define YAHLog(...)
#endif

static inline void YAH_swizzleSelector(Class zclass, SEL originalSelector, SEL swizzledSelector) {
    Method originalMethod = class_getInstanceMethod(zclass, originalSelector);
    Method swizzledMethod = class_getInstanceMethod(zclass, swizzledSelector);
    if (class_addMethod(zclass, originalSelector, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod))) {
        class_replaceMethod(zclass, swizzledSelector, method_getImplementation(originalMethod), method_getTypeEncoding(originalMethod));
    } else {
